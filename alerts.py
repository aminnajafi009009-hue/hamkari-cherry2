...
from keyboards import back_button, fair_use_keyboard
import bot_info
...
async def _check_single_config(bot, cfg):
    ...
    if total:
        ...
        # پلن نامحدود: هشدار مصرف منصفانه بر اساس حجم مصرف‌شده، نه درصد.
        try:
            fair_gb = float(bot_info.get("fair_use_gb") or 0)
        except Exception:
            fair_gb = 0
        if fair_gb > 0 and used >= fair_gb * (1024 ** 3) and not cfg.get("fair_use_alert_sent"):
            await _send_fair_use_alert(bot, user, cfg, fair_gb)
...
async def _send_fair_use_alert(bot, user, cfg, fair_gb):
    text=t("notif_fair_use",plan=cfg.get("plan") or "سرویس",fair_use_gb=f"{fair_gb:g}")
    try:
