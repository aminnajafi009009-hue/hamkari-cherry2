...
TEXT_CATEGORIES = OrderedDict({
    ...
    ('renew_done', '✅ سرویس «{service_name}» با موفقیت تمدید شد.\n\n📦 حجم اضافه‌شده: {volume}\n⏳ زمان اضافه‌شده: {days}'),
...
def _sanitize_service_detail_template(key: str, template):
    """قالب جزئیات سرویس را برای لینک Subscription تمیز می‌کند.

    بعضی نسخه‌های قبلی لینک را به‌صورت `` `{link}` `` داخل Markdown ذخیره/نمایش
    می‌کردند و در نتیجه خود کاراکترهای backtick در خروجی دیده می‌شدند. فقط همان
    wrapper را حذف می‌کنیم و بقیه قالب و Entityهای Premium را دست نمی‌زنیم.
    """
...
def text(key: str, default: str | None = None, **values) -> str:
    if key not in _CACHE:
        ...
        # حذف backtickهای قدیمی دور {link} نباید Entityهای Premium/Custom Emoji
        # متن ویرایش‌شده را از بین ببرد. فقط وقتی قالب به‌دلیل placeholderهای
        # قدیمی و ناسازگار reset شده است، Entityهای قبلی را کنار می‌گذاریم.
        legacy_template = isinstance(raw_template, str) and any(
            token in raw_template
            for token in ("{status}", "{service_name}", "{location}", "{requested_at}",
