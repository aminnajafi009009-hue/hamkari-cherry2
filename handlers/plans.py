...
@router.message(UserStates.waiting_card_purchase_receipt, F.text)
...
async def _start_crypto_invoice(callback,state,renewal=False,plan_key=None):
    ...
    await answer_rich(
        ...
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=t("crypto_asset_usdt"),callback_data="cryptoasset_USDT",style="success")],
            [InlineKeyboardButton(text=t("crypto_asset_ton"),callback_data="cryptoasset_TON",style="primary"),
             InlineKeyboardButton(text=t("crypto_asset_trx"),callback_data="cryptoasset_TRX",style="danger")],
            [InlineKeyboardButton(text=t("renew_pay_back"),callback_data="renew_cancel",style="danger")],
        ]),
    )
...
async def receive_crypto_receipt(message:types.Message,state:FSMContext):
    ...
    await forward_admin_task_message(message.bot,ADMIN_ID,"receipts",message.chat.id,message.message_id)
    hash_text=html.escape(submitted_text) if submitted_text else t("admin_crypto_photo_only")
    admin_text=t(
    ...
    await send_admin_task_message(
        ...
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=t("admin_crypto_approve"),callback_data=f"approvecrypto|{receipt_id}",style="success"),
             InlineKeyboardButton(text=t("admin_crypto_reject"),callback_data=f"rejectcrypto|{receipt_id}",style="danger")]
        ])
    )
    await answer_rich(message,t("renew_card_registered") if data.get("crypto_renewal") else t("receipt_registered"),reply_markup=main_reply_keyboard(message.from_user.id))
    await state.clear()
