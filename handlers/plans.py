...
async def receive_purchase_receipt(message: types.Message, state: FSMContext):
    ...
    if data.get("renew_invoice_id"):
        ...
        await send_admin_task_message(message.bot,ADMIN_ID,"receipts",f"💳 رسید تمدید سرویس\n\n👤 {message.from_user.full_name}\n🆔 {message.from_user.id}\n🔁 {data.get('renew_service_name','سرویس')}\n💰 {invoice['price']:,} تومان",reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[[types.InlineKeyboardButton(text="✅ تأیید تمدید",callback_data=f"approverenew|{receipt_id}",style="success"),types.InlineKeyboardButton(text="❌ رد",callback_data=f"rejectrenew|{receipt_id}",style="danger")]]))
...
async def renew_choose_service(callback: types.CallbackQuery, state: FSMContext):
    user=db.get_user(callback.from_user.id)
    try: cfg_id=int(callback.data.replace("renewcfg_",""))
    ...
    await enrich_configs_with_subscription_names([cfg]); name=cfg.get("_display_name") or cfg.get("plan") or "سرویس"
    live=None
    try: live=await fetch_subscription_info(crypto.decrypt_config(cfg.get("config") or ""))
    except Exception: pass
    if (live or {}).get("total"):
        mode="volume"; prompt=t("renew_volume_prompt"); kb=renew_volume_keyboard()
    else:
        mode="days"; prompt=t("renew_days_prompt"); kb=renew_days_keyboard()
    await state.update_data(renew_cfg_id=cfg_id,renew_service_name=name,renew_mode=mode)
    await edit_rich(callback.message,t("renew_service_title",service_name=name)+"\n\n"+prompt,reply_markup=kb); await callback.answer()
...
async def _renew_prepare_payment(target,state,volume_gb=0,days=0):
    ...
    if not cfg or not user or cfg.get("user_id")!=user["id"]:
        await answer_rich(target,t("service_not_owned")); await state.clear(); return
    try: per_gb=int(float(bot_info.get("renewal_price_per_gb") or 5500))
    except Exception: per_gb=5500
    price=int(round(float(volume_gb or 0)*per_gb))
    if not volume_gb and days:
        try: per_day=int(float(bot_info.get("renewal_price_per_day") or 0))
        except Exception: per_day=0
        price=int(days)*per_day
    await state.update_data(renew_volume_gb=float(volume_gb or 0),renew_days=int(days or 0),renew_price=price)
    await answer_rich(target,t("renew_summary",service_name=data.get("renew_service_name") or cfg.get("plan"),volume=(f"{volume_gb:g} گیگ" if volume_gb else "بدون تغییر"),days=(f"{days} روز" if days else "بدون تغییر"),price=price),reply_markup=renew_payment_keyboard())
...
async def renew_volume_choice(callback: types.CallbackQuery,state:FSMContext):
    ...
    if value=="custom":
        await state.set_state(UserStates.waiting_renew_custom_volume); await answer_rich(callback.message,t("renew_custom_volume_prompt"),reply_markup=back_button("renew_cancel",t("renew_cancel"))); await callback.answer(); return
    await state.set_state(None); await _renew_prepare_payment(callback.message,state,float(value),0); await callback.answer()
...
async def renew_custom_volume(message:types.Message,state:FSMContext):
    value=parse_int_in_range((message.text or "").strip(),51,100000)
    if value is None: await answer_rich(message,t("renew_custom_volume_prompt")); return
    await state.set_state(None); await _renew_prepare_payment(message,state,float(value),0)
...
async def renew_days_choice(callback:types.CallbackQuery,state:FSMContext):
    ...
    if value=="custom":
        await state.set_state(UserStates.waiting_renew_days); await answer_rich(callback.message,t("renew_custom_days_prompt"),reply_markup=back_button("renew_cancel",t("renew_cancel"))); await callback.answer(); return
    await state.set_state(None); await _renew_prepare_payment(callback.message,state,0,int(value)); await callback.answer()

@router.message(UserStates.waiting_renew_days)
async def renew_custom_days(message:types.Message,state:FSMContext):
    value=parse_int_in_range((message.text or "").strip(),1,100000)
    if value is None: await answer_rich(message,t("renew_custom_days_prompt")); return
    await state.set_state(None); await _renew_prepare_payment(message,state,0,value)

@router.callback_query(F.data == "renew_cancel")
async def renew_cancel(callback:types.CallbackQuery,state:FSMContext):
    await state.clear(); await answer_rich(callback.message,t("renew_cancelled"),reply_markup=main_reply_keyboard(callback.from_user.id)); await callback.answer()
...
async def renew_pay_card(callback:types.CallbackQuery,state:FSMContext):
    ...
    text=t("renew_card_invoice",service_name=data.get("renew_service_name") or cfg.get("plan"),volume=(f"{data.get('renew_volume_gb',0):g} گیگ" if data.get("renew_volume_gb") else "بدون تغییر"),days=(f"{int(data.get('renew_days',0))} روز" if data.get("renew_days") else "بدون تغییر"),amount=invoice["price"],card_number=bot_info.get("card_number"),card_holder=bot_info.get("card_holder"))
    await show_menu_with_sticker(callback.bot,callback.message.chat.id,"plan_pay_card",text,parse_mode=None); await callback.answer()
...
async def _start_crypto_invoice(callback,state,renewal=False,plan_key=None):
    ...
    rows="\n".join(f"• {a}: {price/r:.6f} {a}" for a,r,_,_ in assets)
    ...
    await answer_rich(callback.message,t("crypto_choose_asset_intro",plan_name=label,price=price,rows=rows),reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[[types.InlineKeyboardButton(text=t("crypto_asset_usdt"),callback_data="cryptoasset_USDT")],[types.InlineKeyboardButton(text=t("crypto_asset_ton"),callback_data="cryptoasset_TON"),types.InlineKeyboardButton(text=t("crypto_asset_trx"),callback_data="cryptoasset_TRX")],[types.InlineKeyboardButton(text=t("renew_pay_back"),callback_data="renew_cancel")]]))
...
@router.message(UserStates.waiting_crypto_receipt,F.photo)
@router.message(UserStates.waiting_crypto_receipt,F.text)
async def receive_crypto_receipt(message:types.Message,state:FSMContext):
    data=await state.get_data(); invoice=db.consume_invoice(data.get("crypto_invoice_id")) if data.get("crypto_invoice_id") else None
    if not invoice: await answer_rich(message,t("invoice_expired_wait")); await state.clear(); return
    user=db.get_user(message.from_user.id); kind="crypto_renew" if data.get("crypto_renewal") else "crypto_plan"; extra=json.dumps(data.get("crypto_payload") or {},ensure_ascii=False)
    receipt_id=db.create_pending_receipt(kind,str(message.from_user.id),user["id"],data.get("renew_service_name") or data.get("crypto_plan_key") or "پرداخت ارزی",int(data.get("crypto_price") or 0),extra=extra,plan_key=data.get("crypto_plan_key")); db.delete_invoice(invoice["id"])
    await forward_admin_task_message(message.bot,ADMIN_ID,"receipts",message.chat.id,message.message_id)
    await send_admin_task_message(message.bot,ADMIN_ID,"receipts",f"💱 رسید پرداخت ارزی\\n\\n👤 {message.from_user.full_name}\\n🆔 {message.from_user.id}\\n💰 {int(data.get('crypto_price') or 0):,} تومان\\n💵 {data.get('crypto_amount',0):.6f} {data.get('crypto_asset','-')}",reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[[types.InlineKeyboardButton(text="✅ تأیید پرداخت ارزی",callback_data=f"approvecrypto|{receipt_id}",style="success"),types.InlineKeyboardButton(text="❌ رد",callback_data=f"rejectcrypto|{receipt_id}",style="danger")]]))
    await answer_rich(message,t("renew_card_registered") if data.get("crypto_renewal") else t("receipt_registered"),reply_markup=main_reply_keyboard(message.from_user.id)); await state.clear()
...
async def view_config(callback: types.CallbackQuery):
    ...
    try:
        ...
        status_line = t("service_live_active") if live_status == "active" else (t("service_live_expired") if live_status == "expired" else "⚪ وضعیت نامشخص")
        text = t(
            "service_detail_text",
