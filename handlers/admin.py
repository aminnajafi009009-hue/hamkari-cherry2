...
async def admin_config_search_result(message: types.Message, state: FSMContext):
    ...
    for cfg in matches[:50]:
        ...
        buttons.append([InlineKeyboardButton(
            ...
            callback_data=f"svcdetail_{cfg['id']}"
        )])
    buttons.append([InlineKeyboardButton(text="🔙 بازگشت", callback_data="admin_back")])
    kb = types.InlineKeyboardMarkup(inline_keyboard=buttons)
...
def _text_manager_keyboard(category: str | None = None, page: int = 0):
    ...
    buttons.append([InlineKeyboardButton(text="🔙 بازگشت به دسته‌ها", callback_data="admin_texts")])
    return types.InlineKeyboardMarkup(inline_keyboard=buttons)
...
async def admin_botinfo_edit_save(message: types.Message, state: FSMContext):
    ...
    if key == "config_name_prefix":
        ...
        if not cleaned:
            ...
            return
        value = cleaned
    if key.startswith("renewal_category_"):
        try:
            parts = key.split("_")
            ...
            await answer_rich(message, "❌ مقدار نامعتبر است؛ فقط عدد صفر یا بیشتر وارد کنید.")
            return
        await state.clear()
