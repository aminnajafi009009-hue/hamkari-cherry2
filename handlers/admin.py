...
async def _apply_admin_renewal(receipt):
    ...
    ok, panel_data, msg=await vpn_panel.renew_user_additive(cfg["service_id"],volume,days,source=cfg.get("source"))
    ...
    db.update_config(cfg["id"],cfg.get("plan") or "سرویس",cfg.get("config"),expiry=expiry,service_id=cfg.get("service_id"),qr_file_id=cfg.get("qr_file_id"))
...
async def approve_renewal(callback: types.CallbackQuery):
    ...
    volume,days,_=result; db.resolve_pending_receipt_by_id(receipt_id)
    ...
    try: await send_rich(callback.bot,int(receipt["telegram_id"]),user_text("renew_done",service_name=receipt.get("label") or "سرویس",volume=(f"{volume:g} گیگ" if volume else "بدون تغییر"),days=(f"{days} روز" if days else "بدون تغییر")))
...
async def reject_renewal(callback: types.CallbackQuery):
    ...
    if receipt:
        db.resolve_pending_receipt_by_id(receipt_id)
        await _finish_receipt_message(callback.message,"\n\n❌ رد شد.",queue_refresh=lambda:_render_pending_receipts(callback))
        try: await send_rich(callback.bot,int(receipt["telegram_id"]),t("notif_receipt_rejected"))
        except Exception: pass
    await callback.answer("❌ رد شد.")
...
def _pasargad_status_text():
    ...
    dynamic_count=len(db.get_vpn_panels("pasargad"))
    lines.append(f"اتصال: {'✅ فعال' if (PASARGAD_ENABLED or dynamic_count) else '⚪ هیچ پنلی ثبت نشده'}")
    if dynamic_count:
        lines.append("پنل‌های اضافه‌شده: " + "، ".join(str(x.get("name")) for x in db.get_vpn_panels("pasargad")))
    active = vpn_panel.active_panel()
    lines.append(f"پنل VPN فعال فعلی: {vpn_panel.panel_label(active)}")
    return "\n".join(lines)
...
@router.callback_query(F.data == "pasargad_add")
async def pasargad_add_start(callback: types.CallbackQuery, state: FSMContext):
    if not _is_admin(callback.from_user.id): return
    await state.set_state(AdminStates.waiting_vpn_panel_name)
    await answer_rich(callback.message,"➕ نامی برای پنل جدید پاسارگارد بفرست:",reply_markup=admin_back_button())
    await callback.answer()

@router.message(AdminStates.waiting_vpn_panel_name)
async def pasargad_add_name(message: types.Message, state: FSMContext):
    name=(message.text or "").strip()
    if not name: await answer_rich(message,"❌ نام پنل نمی‌تواند خالی باشد."); return
    await state.update_data(vpn_panel_name=name); await state.set_state(AdminStates.waiting_vpn_panel_url)
    await answer_rich(message,"🌐 آدرس Base URL پنل را بفرست (مثلاً https://panel.example.com):")

@router.message(AdminStates.waiting_vpn_panel_url)
async def pasargad_add_url(message: types.Message, state: FSMContext):
    url=(message.text or "").strip().rstrip("/")
    if not url.startswith(("http://","https://")): await answer_rich(message,"❌ آدرس باید با http:// یا https:// شروع شود."); return
    await state.update_data(vpn_panel_url=url); await state.set_state(AdminStates.waiting_vpn_panel_username)
    await answer_rich(message,"👤 یوزرنیم ادمین پنل را بفرست:")

@router.message(AdminStates.waiting_vpn_panel_username)
async def pasargad_add_username(message: types.Message, state: FSMContext):
    value=(message.text or "").strip()
    if not value: await answer_rich(message,"❌ یوزرنیم خالی است."); return
    await state.update_data(vpn_panel_username=value); await state.set_state(AdminStates.waiting_vpn_panel_password)
    await answer_rich(message,"🔐 پسورد ادمین پنل را بفرست:")

@router.message(AdminStates.waiting_vpn_panel_password)
async def pasargad_add_password(message: types.Message, state: FSMContext):
    data=await state.get_data(); password=(message.text or "").strip()
    try: await message.delete()
    except Exception: pass
    if not password: await answer_rich(message,"❌ پسورد خالی است."); return
    panel_id=db.add_vpn_panel(data["vpn_panel_name"],"pasargad",data["vpn_panel_url"],data["vpn_panel_username"],password)
    key=f"pasargad:{panel_id}"
    token=pasargad.set_runtime_panel(db.get_vpn_panel(panel_id))
    try: ok,_,msg=await pasargad.test_connection()
    finally: pasargad.reset_runtime_panel(token)
    if not ok:
        db.delete_vpn_panel(panel_id); await state.clear(); await answer_rich(message,f"❌ اتصال پنل جدید برقرار نشد؛ پنل ذخیره نشد.\n{msg}"); return
    vpn_panel.set_active_panel(key); await state.clear()
    await answer_rich(message,f"✅ پنل «{data['vpn_panel_name']}» اضافه و تست شد و به‌عنوان پنل فعال انتخاب شد.\n\nحالا از «🗂 دسته‌بندی‌های VIP» نگاشت پلن‌ها را روی همین پنل انجام بده.",reply_markup=_admin_panel_kb_for(message.from_user.id))

@router.callback_query(F.data.startswith("pasargaddel_"))
async def pasargad_delete_panel(callback: types.CallbackQuery):
    if not _is_admin(callback.from_user.id): return
    panel_id=int(callback.data.replace("pasargaddel_","")); active=vpn_panel.active_panel()
    if active==f"pasargad:{panel_id}":
        await callback.answer("❌ اول یک پنل دیگر را به‌عنوان فعال انتخاب کن.",show_alert=True); return
    db.delete_vpn_panel(panel_id)
    await edit_rich(callback.message,_pasargad_status_text(),reply_markup=admin_pasargad_menu(_pasargad_status_text())); await callback.answer("✅ پنل حذف شد.")
...
async def admin_pasargad_test(callback: types.CallbackQuery):
    ...
    active=vpn_panel.active_panel()
    token=vpn_panel._panel_runtime(active)
    try: ok, data, msg = await pasargad.test_connection()
    finally: vpn_panel._reset_panel_runtime(token)
    icon = "✅" if ok else "❌"
...
async def admin_panel_choose_set(callback: types.CallbackQuery):
    ...
    active = vpn_panel.active_panel()
    text = (
        f"✅ پنل فعال تغییر کرد: {vpn_panel.panel_label(active)}\n\n"
