...
def _admin_renewal_settings(category_id):
    ...
    if cid <= 0:
        return defaults
    fields = ("mode","price_day","price_gb","min_day","max_day","min_gb","max_gb","day_options","gb_options")
    for field in fields:
...
async def admin_renewal_category(callback: types.CallbackQuery, state: FSMContext):
    ...
    await edit_rich(
        callback.message,
        f"🔁 تنظیمات تمدید\n\n🗂 دسته: {cat['name']}\n\n"
        f"حالت: { {'day':'فقط روز','gb':'فقط گیگ','both':'هم روز + هم گیگ'}.get(st['mode'], 'فقط روز') }\n"
        f"⏱ قیمت روز: {st['price_day']:,} تومان\n"
        f"💾 قیمت گیگ: {st['price_gb']:,} تومان\n"
        f"📏 روز: {st['min_day']} تا {st['max_day'] or 'نامحدود'}\n"
        f"📦 گیگ: {st['min_gb']} تا {st['max_gb'] or 'نامحدود'}\n"
        f"🔘 دکمه‌های روز: {st.get('day_options') or '30,60,90'}\n"
        f"🔘 دکمه‌های گیگ: {st.get('gb_options') or '10,20,50'}",
        reply_markup=admin_renewal_category_menu(category_id),
    )
    await callback.answer()
...
async def admin_renewal_mode(callback: types.CallbackQuery, state: FSMContext):
    try:
        category_id = int(callback.data.replace("renewsetmode_", ""))
    except Exception:
        await callback.answer("❌ دسته نامعتبر است.", show_alert=True)
        return
    await edit_rich(callback.message, "نوع تمدید این دسته را انتخاب کن:", reply_markup=admin_renewal_mode_menu(category_id))
    await callback.answer()
...
async def admin_renewal_options_start(callback: types.CallbackQuery, state: FSMContext):
    parts = callback.data.split("_")
    if len(parts) != 4:
        await callback.answer("❌ گزینه نامعتبر.", show_alert=True); return
    unit, category_id = parts[2], parts[3]
    if unit not in ("day", "gb"):
        await callback.answer("❌ گزینه نامعتبر.", show_alert=True); return
    try: category_id = int(category_id)
    except Exception:
        await callback.answer("❌ دسته نامعتبر.", show_alert=True); return
    st = _admin_renewal_settings(category_id)
    current = st.get(f"{unit}_options") or ("30,60,90" if unit == "day" else "10,20,50")
    await state.update_data(botinfo_key=f"renewal_category_{category_id}_{unit}_options", renewal_category_id=category_id)
    await state.set_state(AdminStates.waiting_botinfo_value)
    label = "روز" if unit == "day" else "گیگ"
    await answer_rich(callback.message, f"🔘 دکمه‌های {label} را با کاما جدا کن.\nمثال: 10,20,50\nمقدار فعلی: {current}", reply_markup=admin_botinfo_field_keyboard(""))
    await callback.answer()
...
async def admin_renewal_numeric_start(callback: types.CallbackQuery, state: FSMContext):
    parts = callback.data.split("_")
    # renewset_price_day_12 / renewset_max_gb_12
    if len(parts) != 4:
        await callback.answer("❌ گزینه نامعتبر.", show_alert=True)
        return
    field, unit, category_id = parts[1], parts[2], parts[3]
    if field not in ("price", "min", "max") or unit not in ("day", "gb"):
        await callback.answer("❌ گزینه نامعتبر.", show_alert=True)
        return
    ...
    await answer_rich(
        callback.message,
        f"✏️ {label} {suffix} را وارد کن.\nمقدار فعلی: {current or 'نامحدود' if field == 'max' else current}",
...
async def admin_botinfo_edit_save(message: types.Message, state: FSMContext):
    ...
    # تنظیمات تمدید کلید مجازی دارند و عضو labels اطلاعات ربات نیستند.
    # این شاخه باید قبل از بررسی key در labels اجرا شود.
    if key and key.startswith("renewal_category_"):
        try:
            parts = key.split("_")
            if len(parts) == 5 and parts[0:2] == ["renewal", "category"] and parts[3] in ("day", "gb") and parts[4] == "options":
                category_id = int(parts[2]); unit = parts[3]
                nums = []
                ...
                if not nums or len(nums) > 3: raise ValueError
                st = _admin_renewal_settings(category_id)
                minimum = int(st.get(f"min_{unit}") or 1)
                maximum = int(st.get(f"max_{unit}") or 0)
                if any(n < minimum or (maximum and n > maximum) for n in nums): raise ValueError
                canonical = ",".join(str(n) for n in nums)
                db.set_setting(f"renewal_category_{category_id}_{unit}_options", canonical)
                try: bot_info.set_renewal_setting(category_id, f"{unit}_options", canonical)
                except Exception: pass
                await state.clear()
                await answer_rich(message, "✅ دکمه‌های انتخاب تمدید به‌روز شد.", reply_markup=admin_renewal_category_menu(category_id))
                return
            if len(parts) != 5 or parts[0:2] != ["renewal", "category"]:
                raise ValueError
            ...
            if field not in ("price", "min", "max") or unit not in ("day", "gb"):
                raise ValueError
            number = int(float(value.replace(",", "").strip()))
            ...
            return
        await state.clear()
        await answer_rich(message, "✅ تنظیمات تمدید به‌روز شد.", reply_markup=admin_renewal_category_menu(category_id))
        return
