...
from keyboards import (
    ...
    admin_botinfo_renewal_categories_menu,
    admin_botinfo_renewal_category_menu,
...
@router.callback_query(F.data == "botinforenewal")
async def admin_botinfo_renewal_categories(callback: types.CallbackQuery, state: FSMContext):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    await state.clear()
    await edit_rich(callback.message, "💰 قیمت تمدید بر اساس دسته‌بندی\n\nدسته‌بندی موردنظر را انتخاب کنید:", reply_markup=admin_botinfo_renewal_categories_menu())
    await callback.answer()


@router.callback_query(F.data.startswith("botinforenewcat_"))
async def admin_botinfo_renewal_category(callback: types.CallbackQuery, state: FSMContext):
    if not _is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    try:
        category_id = int(callback.data.replace("botinforenewcat_", ""))
    except Exception:
        await callback.answer("❌ دسته نامعتبر است.", show_alert=True)
        return
    cat = next((c for c in db.get_vip_categories() if int(c["id"]) == category_id), None)
    if not cat:
    ...
    await edit_rich(callback.message, f"💰 تنظیم قیمت تمدید\n\n🗂 دسته: {cat['name']}\n\n💰 هر گیگ: {gb:,} تومان\n⏱ هر روز: {day:,} تومان", reply_markup=admin_botinfo_renewal_category_menu(category_id))
    await callback.answer()


async def _start_category_renewal_edit(callback: types.CallbackQuery, state: FSMContext, unit: str):
    try:
    ...
    if not cat:
        await callback.answer("❌ دسته پیدا نشد.", show_alert=True)
        return
    label = "قیمت هر گیگ" if unit == "gb" else "قیمت هر روز"
    await state.update_data(botinfo_key=f"renewal_price_category_{category_id}_{unit}", botinfo_renewal_category_id=category_id, botinfo_renewal_unit=unit)
    await state.set_state(AdminStates.waiting_botinfo_value)
    current = bot_info.get_renewal_price(category_id, unit)
    await answer_rich(callback.message, f"✏️ {label} برای «{cat['name']}» را به تومان ارسال کنید.\nمقدار فعلی: {current:,}", reply_markup=admin_botinfo_field_keyboard(""))
    await callback.answer()


@router.callback_query(F.data.startswith("botinforenewgb_"))
async def admin_botinfo_renewal_gb(callback: types.CallbackQuery, state: FSMContext):
    await _start_category_renewal_edit(callback, state, "gb")


@router.callback_query(F.data.startswith("botinforenewday_"))
async def admin_botinfo_renewal_day(callback: types.CallbackQuery, state: FSMContext):
    await _start_category_renewal_edit(callback, state, "day")
...
async def admin_botinfo_edit_save(message: types.Message, state: FSMContext):
    ...
    if key == "config_name_prefix":
        ...
        if not cleaned:
            ...
            return
        value = cleaned
    if key.startswith("renewal_price_category_"):
        try:
            parts = key.split("_")
            ...
            await answer_rich(message, "❌ مبلغ نامعتبر است؛ فقط عدد وارد کنید:")
            return
        bot_info.set_renewal_price(category_id, unit, int(value))
        await state.clear()
        await answer_rich(message, "✅ قیمت تمدید این دسته به‌روز شد.", reply_markup=admin_botinfo_menu())
        return
