...
from utils import is_duplicate_action, now_tehran_naive, parse_int_in_range, TEHRAN_TZ
...
async def panel_renew_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    back_kb = types.InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 بازگشت", callback_data=f"marzbanrenewback_{cfg_id}")]
    ])
    if mode == "day":
