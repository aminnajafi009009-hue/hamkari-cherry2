...
async def marzban_renew_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not cfg or not str(cfg.get("source") or "").startswith(("marzban", "pasargad")) or not cfg.get("service_id"):
    ...
    await answer_rich(callback.message, 
        "حجم اضافه را به گیگابایت وارد کن (عدد 0 یعنی حجم نامحدود):",
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[[types.InlineKeyboardButton(text="🔙 بازگشت", callback_data=f"marzbanrenewback_{cfg_id}")]])
    )
