...
def _admin_renewal_settings_for_config(cfg: dict) -> dict:
    """همان تنظیمات تمدید دسته‌بندی فروشگاه را برای تمدید ادمین می‌خواند."""
    try:
    ...
    if not cid:
        try:
            raw_name = str(cfg.get("plan") or "").split("|", 1)[0].strip()
            cur = db.get_connection().cursor()
            cur.execute("SELECT category_id FROM vip_plans WHERE name = ? LIMIT 1", (raw_name,))
            row = cur.fetchone()
...
async def marzban_renew_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    if mode == "day":
        ...
        await state.set_state(AdminStates.waiting_marzban_renew_volume)
        prompt = "حجم اضافه را به گیگابایت وارد کن:"
    await answer_rich(callback.message, prompt, reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 بازگشت", callback_data=f"marzbanrenewback_{cfg_id}")]]))
...
async def marzban_renew_volume_received(message: types.Message, state: FSMContext):
    ...
    if volume_gb < int(settings.get("min_gb") or 1) or (settings.get("max_gb") and volume_gb > int(settings["max_gb"])):
    ...
    if data.get("marzban_renew_mode") == "gb":
        ...
        return
    await state.set_state(AdminStates.waiting_marzban_renew_days)
    await answer_rich(message, "تعداد روز اضافه را وارد کن:", reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 بازگشت", callback_data=f"marzbanrenewback_{data.get('marzban_renew_cfg_id')}")]]))
...
async def _apply_admin_panel_renew(message: types.Message, state: FSMContext, volume_gb, days):
    ...
    ok, snapshot, msg = await panels.renew_existing_service(panel, cfg["service_id"], volume_gb or 0, days or 0)
    if not ok:
