...
def renew_services_keyboard(configs):
    ...
    for cfg in configs:
        ...
        buttons.append([InlineKeyboardButton(text=t("renew_service_button", service_name=name), callback_data=f"renewcfg_{cfg['id']}", style="primary", icon_custom_emoji_id=db.get_button_custom_emoji_id("renew_service_button"))])
...
def renew_payment_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=t("renew_pay_card"), callback_data="renewpay_card", style="success"), InlineKeyboardButton(text=t("renew_pay_crypto"), callback_data="renewpay_crypto", style="success")],
...
def admin_renewal_categories_menu():
    ...
    for cat in db.get_vip_categories():
        ...
        buttons.append([InlineKeyboardButton(
            ...
            callback_data=f"renewsetcat_{cat['id']}", style="primary",
        )])
    buttons.append([InlineKeyboardButton(text="🔙 بازگشت", callback_data="admin_back", style="primary")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def admin_renewal_category_menu(category_id: int):
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"🧩 بخش تمدید: {mode}", callback_data=f"renewsetmode_{category_id}", style="success")],
        [InlineKeyboardButton(text=f"⏱ قیمت روز: {st['price_day']:,}", callback_data=f"renewset_price_day_{category_id}", style="primary")],
        [InlineKeyboardButton(text=f"💾 قیمت گیگ: {st['price_gb']:,}", callback_data=f"renewset_price_gb_{category_id}", style="primary")],
        [InlineKeyboardButton(text=f"⬇️ حداقل روز: {st['min_day']}", callback_data=f"renewset_min_day_{category_id}", style="primary")],
        [InlineKeyboardButton(text=f"⬆️ حداکثر روز: {lim(st['max_day'], 'روز')}", callback_data=f"renewset_max_day_{category_id}", style="primary")],
        [InlineKeyboardButton(text=f"⬇️ حداقل گیگ: {st['min_gb']}", callback_data=f"renewset_min_gb_{category_id}", style="primary")],
        [InlineKeyboardButton(text=f"⬆️ حداکثر گیگ: {lim(st['max_gb'], 'گیگ')}", callback_data=f"renewset_max_gb_{category_id}", style="primary")],
        [InlineKeyboardButton(text="🔙 بازگشت به دسته‌ها", callback_data="admin_renewal_settings", style="primary")],
        [InlineKeyboardButton(text="🔙 تنظیمات", callback_data="admin_back", style="primary")],
    ])

def admin_renewal_mode_menu(category_id: int):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⏱ فقط روز", callback_data=f"renewsetmodeval_day_{category_id}", style="primary")],
        [InlineKeyboardButton(text="💾 فقط گیگ", callback_data=f"renewsetmodeval_gb_{category_id}", style="primary")],
        [InlineKeyboardButton(text="🧩 هم روز + هم گیگ", callback_data=f"renewsetmodeval_both_{category_id}", style="success")],
        [InlineKeyboardButton(text="🔙 بازگشت", callback_data=f"renewsetcat_{category_id}", style="danger")],
    ])
