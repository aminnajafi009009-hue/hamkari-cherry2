...
# ---------------------------------------------------------------------------
def create_order(user_id: int, plan_key: str | None, plan_name: str, order_type: str, price: int, target_config_id: int | None = None) -> int:
    with transaction() as cur:
        cur.execute(
            """INSERT INTO orders (user_id, plan_key, plan_name, order_type, price, status, target_config_id, created_at)
               VALUES (?, ?, ?, ?, ?, 'pending', ?, ?)""",
            (user_id, plan_key, plan_name, order_type, price, target_config_id, _now()),
        )
        return cur.lastrowid
...
def get_panel_map_for_plan_key(plan_key):
    ...
    return get_panel_plan_map_with_panel('vip_plan',plan['id']) or get_panel_plan_map_with_panel('vip_category',plan['category_id'])



# ---------------------------------------------------------------------------
# مدیریت یکپارچه‌ی نمونه‌های پنل (مرزبان/پاسارگارد)
# ---------------------------------------------------------------------------
def create_vpn_panel(panel_type: str, name: str, base_url: str, api_key: str | None = None,
                     username: str | None = None, password: str | None = None,
                     auth_method: str = "userpass") -> int:
    if panel_type not in ("marzban", "pasargad"):
        raise ValueError("نوع پنل نامعتبر است")
    with transaction() as cur:
        cur.execute(
            """INSERT INTO vpn_panels
               (panel_type,name,base_url,api_key,username,password,auth_method,enabled,sort_order,created_at)
               VALUES (?,?,?,?,?,?,?,?,0,?)""",
            (panel_type, name, base_url.rstrip("/"), api_key, username, password, auth_method, 1, _now()),
        )
        return cur.lastrowid
...
def get_panel_plan_map_with_panel(scope: str, scope_id: int):
    cur=get_connection().cursor()
    cur.execute(
        """SELECT m.*, p.panel_type, p.name AS panel_name, p.base_url, p.api_key,
                  p.username, p.password, p.auth_method, p.enabled
